...
@ThreadSafe
public class HashBuilderStarJoinOperator
        implements Operator
{
    private static final Logger log = Logger.get(HashBuilderStarJoinOperator.class);

    public static class HashBuilderStarJoinOperatorFactory
            implements OperatorFactory
    {
        ...
        private int starJoinBuildTableIndex;

        public HashBuilderStarJoinOperatorFactory(
                int operatorId,
                ...
                int starJoinBuildTableIndex)
        {
            ...
            this.starJoinPageIndex = requireNonNull(starJoinPageIndex, "starJoinPageIndex is null");
            this.spillEnabled = spillEnabled;
            this.singleStreamSpillerFactory = requireNonNull(singleStreamSpillerFactory, "singleStreamSpillerFactory is null");

            this.expectedPositions = expectedPositions;
            this.enforceBroadcastMemoryLimit = enforceBroadcastMemoryLimit;
            this.starJoinBuildTableIndex = starJoinBuildTableIndex;
        }

        @Override
        public HashBuilderStarJoinOperator createOperator(DriverContext driverContext)
        {
            checkState(!closed, "Factory is already closed");
            OperatorContext operatorContext = driverContext.addOperatorContext(operatorId, planNodeId, HashBuilderStarJoinOperator.class.getSimpleName());
            ...
            return new HashBuilderStarJoinOperator(
                    operatorContext,
                    lookupSourceFactory,
                    partitionIndex,
                    outputChannels,
    ...
    private final StarJoinPageIndex pagesIndexFactory;

    public HashBuilderStarJoinOperator(
            ...
            List<JoinFilterFunctionFactory> searchFunctionFactories,
            int expectedPositions,
            StarJoinPageIndex pagesIndexFactory,
            boolean spillEnabled,
            SingleStreamSpillerFactory singleStreamSpillerFactory,
            boolean enforceBroadcastMemoryLimit,
            int starJoinBuildTableIndex,
            int partitionNum)
    {
        ...
        this.index = pagesIndexFactory.newPagesIndex(starJoinBuildTableIndex, partitionIndex, lookupSourceFactory.getTypes(), expectedPositions);
    ...
    private void spillInput(Page page)
    {
        ...
        if (totalSpilledBytes > maxUserMemoryBytes) {
            String additionalInfo = format("Spilled: %s, Operator: %s", succinctBytes(totalSpilledBytes), HashBuilderStarJoinOperator.class.getSimpleName());
    ...
    private ListenableFuture<?> spillIndex()
    {
        ...
        if (indexEstimatedSize > maxUserMemoryBytes) {
            ...
            String additionalInfo = format("Estimated Spilled: %s, Operator: %s", succinctBytes(indexEstimatedSize), HashBuilderStarJoinOperator.class.getSimpleName());
            throw exceededLocalUserMemoryLimit(succinctBytes(maxUserMemoryBytes), additionalInfo, false, Optional.empty(), ErrorCause.UNKNOWN);
        }
    ...
    {
        LookupSourceSupplier partition = index.createLookupSourceSupplier(operatorContext.getSession(), hashChannels, preComputedHashChannel, filterFunctionFactory, sortChannel,
                searchFunctionFactories, Optional.of(outputChannels), pagesIndexFactory, starJoinBuildTableIndex, partitionIndex);
