...
public final class FbInvokePythonUdfFunction
        extends SqlScalarFunction
{
    // TODO design new variadic functions binding mechanism that will allow to produce VARCHAR(x) where x < MAX_LENGTH.
    public static final FbInvokePythonUdfFunction FB_VARCHAR_CONCAT = new FbInvokePythonUdfFunction(createUnboundedVarcharType().getTypeSignature(), "concatenates given strings");

    public static final FbInvokePythonUdfFunction FB_VARBINARY_CONCAT = new FbInvokePythonUdfFunction(VARBINARY.getTypeSignature(), "concatenates given varbinary values");

    private final String description;

    private FbInvokePythonUdfFunction(TypeSignature type, String description)
    {
        super(new Signature(
                QualifiedObjectName.valueOf(DEFAULT_NAMESPACE, "fb_invoke_python"),
                FunctionKind.SCALAR,
                ImmutableList.of(),
                ImmutableList.of(),
                type,
