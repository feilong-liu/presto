...
@ScalarFunction(value = "find", deterministic = true)
public class ArrayFindWithOffsetFunction
{
    protected ArrayFindWithOffsetFunction() {}
            ...
            SliceToBooleanFunction function)
    {
        ...
        for (int i = startPosition; i < arrayBlock.getPositionCount() && i >= 0; i += increment) {
            Slice element = null;
            if (!arrayBlock.isNull(i)) {
                element = (Slice) elementType.getSlice(arrayBlock, i);
            }
            Boolean match = function.apply(element);
            if (TRUE.equals(match)) {
                return element;
            ...
            LongToBooleanFunction function)
    {
        ...
        for (int i = startPosition; i < arrayBlock.getPositionCount() && i >= 0; i += increment) {
            Long element = null;
            if (!arrayBlock.isNull(i)) {
                element = (Long) elementType.getLong(arrayBlock, i);
            }
            Boolean match = function.apply(element);
            if (TRUE.equals(match)) {
                return element;
            ...
            DoubleToBooleanFunction function)
    {
        ...
        for (int i = startPosition; i < arrayBlock.getPositionCount() && i >= 0; i += increment) {
            Double element = null;
            if (!arrayBlock.isNull(i)) {
                element = (Double) elementType.getDouble(arrayBlock, i);
            }
            Boolean match = function.apply(element);
            if (TRUE.equals(match)) {
                return element;
            ...
            BooleanToBooleanFunction function)
    {
        ...
        for (int i = startPosition; i < arrayBlock.getPositionCount() && i >= 0; i += increment) {
            ...
            if (!arrayBlock.isNull(i)) {
                element = (Boolean) elementType.getBoolean(arrayBlock, i);
            }
            Boolean match = function.apply(element);
            if (TRUE.equals(match)) {
                return element;
