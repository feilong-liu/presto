        ...
        implements PlanOptimizer
{
    ...
    public PlanNode optimize(PlanNode plan, Session session, TypeProvider types, VariableAllocator variableAllocator, PlanNodeIdAllocator idAllocator, WarningCollector warningCollector)
    {
        if (isRewriteExpressionWithConstantEnabled(session)) {
            return plan.accept(new Rewriter(idAllocator, functionAndTypeManager), null).getPlanNode();
        }
        return plan;
    }
            ...
            extends InternalPlanVisitor<PlanNodeWithConstant, Void>
    {
        ...
        // This rule only extracts the constant variables and the optimizations are in separated optimization rules.
        // We add a projection node with constant assignments for plan nodes which have optimizations.
        private PlanNodeWithConstant planAndReplace(PlanNode node, boolean keepConstantConstraint, boolean addConstantProjection)
        {
            ...
            if (!addConstantProjection) {
                newSources.addAll(children.stream().map(PlanNodeWithConstant::getPlanNode).collect(toImmutableList()));
            }
            else {
                for (PlanNodeWithConstant planNodeWithConstant : children) {
                    ...
                    assignmentsBuilder.putAll(child.getOutputVariables().stream().collect(toImmutableMap(identity(), x -> constantExpressionMap.containsKey(x) ? constantExpressionMap.get(x) : x)));
                    newSources.add(new ProjectNode(idAllocator.getNextId(), child, assignmentsBuilder.build()));
                }
            }
        ...
        {
            return planAndReplace(node, false, false);
        }
        ...
        public PlanNodeWithConstant visitFilter(FilterNode node, Void context)
        {
            ...
            if (!rewrittenChild.getConstantExpressionMap().isEmpty()) {
                ...
                outputConstantMap.putAll(rewrittenChild.getConstantExpressionMap());
            }
                    ...
                    if (isSupportedType(argument0) && isSupportedType(argument1)) {
                        if ((argument0 instanceof VariableReferenceExpression && argument1 instanceof ConstantExpression)
                            ...
                            if (newConstantMap.containsKey(variable) && !newConstantMap.get(variable).equals(constant)) {
                                return new PlanNodeWithConstant(newFilterNode.replaceChildren(ImmutableList.of(rewrittenChild.getPlanNode())), ImmutableMap.of());
                            }
            ...
            outputConstantMap.putAll(newConstantMap);
            return new PlanNodeWithConstant(newFilterNode.replaceChildren(ImmutableList.of(rewrittenChild.getPlanNode())), outputConstantMap.build());
        }
        ...
        public PlanNodeWithConstant visitProject(ProjectNode node, Void context)
        {
            ...
            for (Map.Entry<VariableReferenceExpression, RowExpression> entry : newProjectNode.getAssignments().getMap().entrySet()) {
                if (entry.getValue() instanceof ConstantExpression && isSupportedType(entry.getKey()) && isSupportedType(entry.getValue())) {
                    newConstantMap.put(entry.getKey(), (ConstantExpression) entry.getValue());
                }
            }

            return new PlanNodeWithConstant(newProjectNode.replaceChildren(ImmutableList.of(rewrittenChild.getPlanNode())), newConstantMap.build());
        }
        ...
        public PlanNodeWithConstant visitJoin(JoinNode node, Void context)
        {
            ...
            return new PlanNodeWithConstant(node.replaceChildren(ImmutableList.of(rewrittenLeft.getPlanNode(), rewrittenRight.getPlanNode())), outputConstantMap.build());
        }
        ...
        public PlanNodeWithConstant visitUnion(UnionNode node, Void context)
        {
            ...
            return new PlanNodeWithConstant(node.replaceChildren(rewrittenSources.stream().map(PlanNodeWithConstant::getPlanNode).collect(toImmutableList())), outputConstantMap.build());
        }
        ...
        {
            return planAndReplace(node, true, true);
        }
        ...
        {
            return planAndReplace(node, true, false);
        }
    ...
    private static class PlanNodeWithConstant
    {
        ...
        public PlanNodeWithConstant(PlanNode planNode, Map<VariableReferenceExpression, ConstantExpression> constantExpressionMap)
        {
            ...
            this.constantExpressionMap = ImmutableMap.copyOf(constantExpressionMap);
        }
