...
public class RowExpressionRewriteRuleSet
{
            ...
            implements Rule<ValuesNode>
    {
        ...
        public Result apply(ValuesNode valuesNode, Captures captures, Context context)
        {
            ...
            if (anyRewritten) {
                return Result.ofPlanNode(new ValuesNode(valuesNode.getSourceLocation(), valuesNode.getId(), valuesNode.getOutputVariables(), rows.build(), valuesNode.getTableName()));
            }
            return Result.empty();
        }
    }
